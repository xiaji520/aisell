...
function showMenu(e, rowIndex, rowData) {
    //选中这个行
    $("#purchasebillitemGrid").datagrid("selectRow", rowIndex);
    //第0个位置的面板不支持相应功能
    e.preventDefault();
    $('#gridMenu').menu('show', {
        left: e.pageX,
...
$(function () {
    ...
    xiaji = {
        ...
        save() {
            ...
            //获到id的值
            var purchasebillitemId = $("#purchasebillitemId").val();
            if (purchasebillitemId) {
                url = "/purchasebillitem/update?cmd=_upd_";
            }
            //easyui的form
        ...
        update() {
            //获取到选中的那一行数据
            var row = purchasebillitemGrid.datagrid("getSelected");
        ...
        delMore() {
            //获取选中数据
            var rows = purchasebillitemGrid.datagrid('getSelections');
            //如果没有选中给提示 选中就是否确定删除
            if (rows.length == 0) {
                $.messager.alert('警告', '请选中再删除!', 'warning');
                return;
